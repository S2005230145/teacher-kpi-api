...
public class AdminConfigManager extends BaseAdminSecurityController {
    ...
    public CompletionStage<Result> listParamConfig(int page, String key) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ExpressionList<AdminConfig> expressionList = AdminConfig.find.query().where().eq("enable", true);
            if (!ValidationUtil.isEmpty(key)) expressionList.icontains("key", key);
            PagedList<AdminConfig> pagedList = expressionList.orderBy().desc("id").orderBy().asc("key")
                    .setFirstRow(queryPage * BusinessConstant.PAGE_SIZE_20)
                    .setMaxRows(BusinessConstant.PAGE_SIZE_20)
                    .findPagedList();
            List<AdminConfig> list = pagedList.getList();
    ...
    public CompletionStage<Result> getParamConfig(int configId) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            AdminConfig config = AdminConfig.find.query().where().eq("id", configId)
                    .eq("enable", true)
                    .setMaxRows(1)
                    .findOne();
    ...
    public CompletionStage<Result> addParamConfig(Http.Request request) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            AdminConfig param = Json.fromJson(requestNode, AdminConfig.class);
            ...
            AdminConfig config = AdminConfig.find.query().where().eq("key", param.key).setMaxRows(1).findOne();
            if (null != config) return okCustomJson(CODE40003, "该配置的KEY已存在");
            param.setUpdateTime(dateUtils.getCurrentTimeByMills());
    ...
    public CompletionStage<Result> updateParamConfig(Http.Request request, int configId) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            AdminConfig config = AdminConfig.find.byId(configId);
            ...
            if (!ValidationUtil.isEmpty(param.key)) {
                AdminConfig existConfig = AdminConfig.find.query().where().eq("key", param.key).setMaxRows(1).findOne();
            ...
            config.setUpdateTime(dateUtils.getCurrentTimeByMills());
            config.save();
            updateParamConfigCache();
            return okJSON200();
        });
    ...
    public CompletionStage<Result> delParamConfig(Http.Request request) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            AdminConfig config = AdminConfig.find.byId(id);
